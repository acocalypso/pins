...
namespace NINA.Test.Sequencer.Logic {
    public class SymbolBrokerTest {
        ...
        public void SymbolBroker_SymbolEvents_ReportAddUpdateAndRemove() {
            ...
            // Filter to events raised by our own provider: the broker's background
            // ConditionWatchdog (started in the SymbolBroker constructor) refreshes
            // core "NINA" symbols like LocalSiderealTime concurrently, which would
            // otherwise flakily leak extra entries into these lists.
            var addedEvent = added.Should().ContainSingle(x => x.ProviderName == "Events").Which;
            addedEvent.ChangeKind.Should().Be(SymbolChangeKind.Added);
            addedEvent.ProviderName.Should().Be("Events");
            addedEvent.Key.Should().Be("Value");
            addedEvent.QualifiedKey.Should().Be("Events_Value");
            addedEvent.OldValue.Should().BeNull();
            addedEvent.NewValue.Should().Be(1);

            var updatedEvent = updated.Should().ContainSingle(x => x.ProviderName == "Events").Which;
            updatedEvent.ChangeKind.Should().Be(SymbolChangeKind.Updated);
            updatedEvent.OldValue.Should().Be(1);
            updatedEvent.NewValue.Should().Be(2);

            var removedEvent = removed.Should().ContainSingle(x => x.ProviderName == "Events").Which;
            removedEvent.ChangeKind.Should().Be(SymbolChangeKind.Removed);
            removedEvent.OldValue.Should().Be(2);
            removedEvent.NewValue.Should().BeNull();
        }
